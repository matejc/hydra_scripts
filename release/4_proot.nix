...
let
  ...
  runCommand = pkgs.writeText "runCommand" ''
    ...
    timeout ${timeout} ${pkgs.proot}/bin/proot -S "$PROOT_DIR" -b /bin/sh -b ${pkgs.busybox} -b ${pkgs.nix} -b ${pkgs.gnutar} -b ${pkgs.bzip2} -b ${<hydra_scripts>} -b ${<nixpkgs>} -b ${buildScript} ${extraPRootArgs} ${buildScript}/bin/build.sh
