...
let
  ...
  config = {
    ...
    packageOverrides = pkgs : rec {
      ...
      buildPerlCrossPackage = import ../overrides/buildPerlPackage-cross.nix (pkgs.makeStdenvCross pkgs.stdenv crosssystem binutilsCross pkgs.gccCrossStageStatic) pkgs.perl520 perlCross glibcCross pkgs busybox hydra_scripts;
      ...
      nix.crossDrv = pkgs.lib.overrideDerivation (pkgs.nix.override { perl = pkgs.perl520; perlPackages = perl520Packages; }).crossDrv (oldAttrs: {
        #preBuild = ''
        #  ${pkgsNoOverrides.findutils}/bin/find . -type f -exec sed -i -e 's|${pkgs.perl520}|${perlCross}|g' {} \;
        #  ${pkgsNoOverrides.findutils}/bin/find . -type f -exec sed -i -e "s|${perl520Packages.DBI}/${pkgs.perl520.libPrefix}|`realpath ${perlCrossPackages.DBI}/${perlCross.libPrefix}/*/*/`|g" {} \;
        #  ${pkgsNoOverrides.findutils}/bin/find . -type f -exec sed -i -e "s|${perl520Packages.DBDSQLite}/${pkgs.perl520.libPrefix}|`realpath ${perlCrossPackages.DBDSQLite}/${perlCross.libPrefix}/*/*/`|g" {} \;
        ...
        #'';
        postInstall = ''
          ${pkgsNoOverrides.findutils}/bin/find $out -type f -exec sed -i -e 's|/bin/sh|${pkgs.bashInteractive.crossDrv}/bin/bash|g' {} \;
