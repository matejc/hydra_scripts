...
let
  ...
  vmBuildCommands = ''
    ...
    nix-build ${<hydra_scripts/release/vm_build_repo.nix>} -A ${esc package_name} --argstr nixpkgs ${<nixpkgs>} --argstr hydra_scripts ${<hydra_scripts>} --argstr prefix ${prefixDir} --argstr build_inputs_str "${build_inputs_str}" --argstr system ${system} --argstr package_name "${esc package_name}" --argstr package_repo "${<package_repo>}" --argstr build_command "${build_command}" --argstr dist_command "${dist_command}" --argstr check_command "${check_command}" --argstr dist_path "${esc dist_path}" --argstr docs_path "${esc docs_path}" --argstr cov_command "${cov_command}" --argstr source_files "${source_files}" --argstr binary_files "${binary_files}" --argstr install_command "${install_command}" --argstr with_vnc_command "${with_vnc_command}" -vvv --show-trace
