...
let
  ...
  let
    ...
    build_env = pkgs.buildEnv {
      ...
      paths = parsed_buildins ++ [ pkgs.tree pkgs.which pkgs.stdenv pkgs.binutils pkgs.git pkgs.perl pkgs.cacert ] ++ (if with_vnc_command == "" then [] else [ pkgs.nix pkgs.tightvnc pkgs.xorg.fontmiscmisc pkgs.xorg.fontcursormisc pkgs.busybox pkgs.xlibs.libX11 pkgs.xorg.xorgserver ]);
      pathsToLink = [ "/" ];
