...
perl.stdenv.mkDerivation (
  ...
  //
  {
    ...
    preBuild = ''
      ...
      rm $GCCBIN/ld
      echo -e "#!${pkgs.stdenv.shell} -x\n\
      ${pkgs.gccCrossStageStatic}/bin/${pkgs.stdenv.cross.config}-ld \$(cat <<< \$@ | sed -e 's|-Wl,-Bsymbolic|-Bsymbolic|g' -e 's|${perlCross.stdenv.gcc.libc}|${glibcCross}|g' ${sedCrossDrvs buildInputsOrg} -e 's|$PERLLIBDIR|$PERLCROSSLIBDIR|g') -lc $LD_EXTRA_OPTIONS" > $GCCBIN/ld
      chmod +x $GCCBIN/ld
