...
perl.stdenv.mkDerivation (
  ...
  //
  {
    ...
    preBuild = ''
      ...
      ${pkgs.gccCrossStageStatic}/bin/${pkgs.stdenv.cross.config}-gcc -Wl,-dynamic-linker,$INTERPRETER \$(cat <<< \$@ | sed -e 's|-fstack-protector||g' -e 's|${perlCross.stdenv.gcc.libc}|${glibcCross}|g' ${sedCrossDrvs buildInputsOrg} -e 's|$PERLLIBDIR|$PERLCROSSLIBDIR|g') -lc -lgcc -lgcc_s $GCC_EXTRA_OPTIONS" > $GCCBIN/gcc
      chmod +x $GCCBIN/gcc

      rm $GCCBIN/ld
      echo -e "#!${pkgs.stdenv.shell} -x\n\
      ${pkgs.gccCrossStageStatic}/bin/${pkgs.stdenv.cross.config}-ld \$(cat <<< \$@ | sed -e 's|-fstack-protector||g' -e 's|-Wl,-Bsymbolic|-Bsymbolic|g' -e 's|${perlCross.stdenv.gcc.libc}|${glibcCross}|g' ${sedCrossDrvs buildInputsOrg} -e 's|$PERLLIBDIR|$PERLCROSSLIBDIR|g') -lc -lgcc -lgcc_s $LD_EXTRA_OPTIONS" > $GCCBIN/ld
      chmod +x $GCCBIN/ld
